...
const stockBadge = (raw) => {
  ...
  if (s === "так" || s === "yes" || s === "in stock" || s === "instock" || s === "available")
    return { label: "В наявності" };
  if (s === "ні" || s === "no" || s === "out of stock")
    return { label: "Немає" };
  if (s.includes("відшив") || s.includes("sew"))
    return { label: "Відшиваємо" };
  if (s.includes("під замов") || s.includes("preorder") || s.includes("made to order"))
    return { label: "Під замовлення" };
  return { label: "Немає" };
};

// простий формат чисел (грн). Валюти поки прибрали — повернемось пізніше
const fmt = (n) =>
  n == null || n === "" ? "" : new Intl.NumberFormat("uk-UA").format(Number(n));
...
const makeId = (t) =>
  String(t || "").toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9_-]+/g, "");
const splitList = (v) =>
  String(v || "")
    .replace(/\u00A0/g, " ")
    .split(/[;,]/)
...
function buildAutoMap(headers = []) {
  ...
  return {
    ...
    priceBulk: pick(/гурт.*30/i, /30\+/i, /bulk/i, /гурт/i),
    priceBuyout: pick(/викуп.*100/i, /100\+/i, /buyout/i, /викуп/i),
...
function mapRow(r, map) {
  ...
  const row = {
    ...
    priceBulk: toNum(get("priceBulk", ["гурт", "гурт 30+", "bulk"])),
    priceBuyout: toNum(get("priceBuyout", ["викуп", "викуп 100+", "buyout"])),
...
function explodeVariants(row) {
  ...
  const out = [];
  for (const s of S) for (const c of C) for (const k of K) out.push({ ...row, size: s, color: c, protectionClass: k });
  return out;
}
...
function buildProducts(rows) {
  ...
  return Array.from(map.values()).map((p) => {
    ...
    return {
      ...
      classesBySizeColor: Object.fromEntries(
        Object.entries(classesBySizeColor).map(([k, set]) => [k, Array.from(set)])
      ),
    };
  });
}
...
export default function MOLLIUAPrice({
    ...
    (async () => {
      try {
        ...
        const user = Object.fromEntries(
          Object.entries(mapFields || {}).map(([k, v]) => [k, String(v || "").toLowerCase()])
        );
        const eff = { ...auto, ...user };
  ...
  const categories = useMemo(() => {
    ...
    for (const p of products) {
      if (!set.has(p.categoryKey)) set.set(p.categoryKey, p.category);
    }
  ...
  const topLink = { color: "var(--text)", textDecoration: "none", opacity: 0.9 };

  return (
    <div className="molli" style={{ padding: 20, minHeight: "100vh" }}>
      {/* TOP BAR */}
      <div className="molli-topbar">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src={logoUrl} alt="MOLLI" style={{ height: 50, display: "block" }} />
        </div>

        <div className="contacts" style={{ fontSize: 14 }}>
          {/* Номер 1 */}
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <a href="tel:+380962019665" style={topLink}>
              +380 96 201 96 65
            </a>
          ...
          {/* Номер 2 */}
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <a href="tel:+380997327413" style={topLink}>
              +380 99 732 74 13
            </a>
            <a href="https://t.me/MolliOpt" target="_blank" rel="noreferrer" style={{ color: "#229ED9" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M9.04 15.314l-.376 5.305c.538 0 .771-.23 1.05-.503l2.521-2.414 5.224 3.82c.958.528 1.642.251 1.912-.886l3.463-16.177.001-.001c.308-1.436-.519-2.002-1.45-1.65L1.13 9.58c-1.402.545-1.381 1.329-.239 1.681л5.748 1.794L19.343 6.27c.67-.44 1.28-.196.777.245" />
              </svg>
            </a>
            <a href="viber://chat?number=%2B380997327413" style={{ color: "#7360F2" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.207 2H5.793C3.15 2 1 4.15 1 6.793v10.414C1 19.85 3.15 22 5.793 22h7.657л3.793 2 .486-2h.478C20.85 22 23 19.85 23 17.207V6.793C23 4.15 20.85 2 18.207 2zm-1.728 14.743c-.236.629-1.365 1.157-1.865 1.157-.5 0-1.079-.236-2.007-.7-1.167-.579-2.383-1.424-3.471-2.511-1.09-1.088-1.934-2.303-2.512-3.471-.464-.928-.7-1.507-.7-2.007 0-.5.529-1.63 1.157-1.865.203-.076.436-.048.592.108l1.317 1.317c.149.149.193.375.112.572-.09.217-.338.77-.46 1.02-.1.205-.081.446.058.635.289.395.708.914 1.241 1.447.533.533 1.052.952 1.447 1.241.189.139.43.158.635.058.25-.122.804-.37 1.02-.46.197-.081.423-.037.572.112l1.317 1.317c.156.156.184.389.108.592z" />
              </svg>
            </a>
          </div>
          <a href="https://molliua.com" target="_blank" rel="noreferrer" style={topLink}>
            molliua.com
          </a>
        </div>
      </div>
      ...
      <div className="molli-cats">
        ...
        >
          {[
            ...
            { key: "drop", label: "Дроп" },
            { key: "bulk", label: "Гурт" },
            { key: "buyout", label: "Викуп" },
          ].map((p) => (
            <button
              key={p.key}
              onClick={() => setPriceMode(p.key)}
              className={"chip" + (priceMode === p.key ? " active" : "")}
              style={{ border: "none" }}
            >
              {p.label}
            </button>
      ...
      {loading && <div style={{ color: "var(--muted)" }}>Завантаження…</div>}
      {error && <div style={{ color: "#f66" }}>Помилка фіду: {String(error)}</div>}

      {/* сітка карток */}
      <div className="molli-grid">
        {filtered.map((p) => {
          const sizeSel = selectedSize[p.id] ?? (p.sizes[0] || "");
          const availColors = p.colorsBySize[sizeSel || "_"] || p.colors;
          const colorSel =
            (selectedColor[p.id] && availColors.includes(selectedColor[p.id])) ? selectedColor[p.id] : (availColors[0] || "");
          const keySC = `${sizeSel || "_"}::${colorSel || "_"}`;
          const availClasses = p.classesBySizeColor[keySC] || p.classes;
          const classSel =
            (selectedClass[p.id] && availClasses.includes(selectedClass[p.id])) ? selectedClass[p.id] : (availClasses[0] || "");
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ color: "var(--muted)", marginTop: 20 }}>
          Нічого не знайдено. Зміни фільтри або пошук.
        </div>
      )}
      ...
      {openProduct && (
        <ProductModal
          ...
          prevImage={() =>
            setGalleryIdx((i) => (i - 1 + (openProduct.photos?.length || 1)) % (openProduct.photos?.length || 1))
          }
